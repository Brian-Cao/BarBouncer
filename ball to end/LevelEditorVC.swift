...
class LevelEditorVC: UIViewController, EditorPresentingDelegate{
    ...
    @objc func addBreakBar(sender: UIButton!){
        levelEditor.addBar(bar: BreakBar(x: 0, y: -150, hardness: 1, angle: .right, size: .small))
    }
    ...
    @objc func printGameData(sender: UIButton!){
        print(levelEditor.gameData.print())
    }
